...
def ensure_cookie() -> Response | None:
    ...
    resp = make_response(render_template('template.html', code=code, compile_answered=True))
    ...
    if not compiles:
        if return_code != 0:
            if request.method == 'GET':
                return render_template('template.html', code=code, not_comp=True, textarea=code, compile_answered=None, correct_choice=True, stdout=stdout)
            ...
            if result.returncode != 0:
                return render_template('template.html', code=code, not_comp=True, textarea=source, compile_answered=None, correct_choice=None, stdout=stdout, textarea_answered=True, correct_answer=None, answer_stdout=result.stdout)
            else:
                return render_template('template.html', code=code, not_comp=True, textarea=source, compile_answered=None, correct_choice=None, stdout=stdout, textarea_answered=True, correct_answer=True, answer_stdout=result.stdout)
        else:
            return render_template('template.html', code=code, comp=True, compile_answered=None, correct_choice=None, stdout=stdout)
    else:
        ...
        if return_code == 0:
            ...
            if output.strip() == result.strip():
                return render_template('template.html', code=code, comp=True, correct_choice=True, correct_answer=True, textarea_answered=True, answer_stdout=result, answered_value=output)
            else:
                return render_template('template.html', code=code, comp=True, correct_choice=True, correct_answer=None, textarea_answered=True, answer_stdout=result, answered_value=output)
        else:
...
def index():
    ...
    return make_response(render_template('template.html', code=code, compile_answered=True))
